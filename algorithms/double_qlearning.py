    ...
    def act(self, reward, state, learning, done):
        ...
        if not done:
            ...
            if self.prev_state is not None and learning:
                self._learn(reward, state, done, qvalues1, qvalues2)
            ...
            self._learn(reward, None, done, None, None)
        else:
            pass

    def _learn(self, reward, state, done, qvalues1, qvalues2):
        ...
        if done:
            ...
            if selector:
                best_action = self.policy.best_action(qvalues1)
                max_q_value = qvalues2[best_action]
            else:
                best_action = self.policy.best_action(qvalues2)
                max_q_value = qvalues1[best_action]
